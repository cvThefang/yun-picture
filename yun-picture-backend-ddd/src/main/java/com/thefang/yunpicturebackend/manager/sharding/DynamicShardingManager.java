package com.thefang.yunpicturebackend.manager.sharding;
...
/**
 * @Description ShardingSphere 动态分表管理器
 * 注意：这边为了方便部署和 减少对公共空间的代码（spaceId=null => 0）的改造（所有增删改查）故不启动该功能。
 * 若需要启动，请在 YunPictureBackendApplication exclude去掉 然后将该类启动即可。
 * 全局搜索 ShardingSphereStart 两个地方，注释掉即可。
 *
 * @Author Thefang
 * @Create 2025/1/21
 */
...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                // 确保前缀合法
                .map(tableName -> "shark_pic." + tableName)
